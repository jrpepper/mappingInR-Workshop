...
Below is the raw code for the workshop: 'Creating Interactive Maps in R with ggMap + Leaflet'. I recommend using the [Markdown text](mapping-in-r.html), as it has all of the maps printed out as figures, but just having blocks of the code may be more useful if you want to highlight and run through all the code at once.
...
##Setup
# This workshop/tutorial will walk you through the basics of using the Leaflet mapping package in R. To begin, lets set up our packages and environment.
...
###ONLY RUN THESE IF YOU HAVEN'T PREVIOUSLY INSTALLED THESE PACKAGES
...
# 3. set the view of the map: `setView(lng = -122.3, lat = 37.9, zoom=5)`
# 
...
```{r, message=FALSE}
#make raster
sSp <- as(SpatialPoints(berkeleyCrime[,c("long","lat")]), "ppp")  # convert points to pp class
Dens <- density(sSp, adjust = 0.3)  # create density object
r <- raster(Dens)
crs(r) <- sp::CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
leaflet() %>% addProviderTiles("CartoDB.Positron") %>%
  addCircleMarkers(data=berkeleyCrime[1:1000,], radius=2, fillColor="red", color=F) %>%
  addRasterImage(r, opacity=0.7, colors=pal.act)
```
...
#Ive also added a marker using an icon to the subset of all peaks that are over 10,000 feet. To do this, we first set the icon setting using the function `icons()` and then using`addMarkers` with the subset of peaks `data=subset(peaks, peaks@data$ele>10000)`
